...
class TrustMetric:
    ...
    def estimate(self, z, rx, gamma):
        """Eq. (5): AGE-AWARE weighted least squares. All detectors must use
        this same estimate -- an unmatched baseline fakes ~0.017 AUC."""
        idx = np.flatnonzero(rx)
        Hr, w = self.H[idx], gamma[idx] / self.s2[idx]
        A = Hr.T @ (Hr * w[:, None])
        if self.cfg.ridge:
            A = A + np.eye(self.n) * self.cfg.ridge
        try:
            return np.linalg.solve(A, Hr.T @ (w * np.asarray(z, float)[idx])), True
        except np.linalg.LinAlgError:
            return np.linalg.lstsq(A, Hr.T @ (w * np.asarray(z, float)[idx]), rcond=None)[0], False
