# PAPER 1 V5 MECHANICAL-VALIDATION OVERLAY
#
# Purpose: estimator/schema/mechanical validation only.
# Output prefix: paper1_v5mv_
...
function Test-CompletedCell {
    ...
    $twin = Get-Content -LiteralPath "$root\twin\meta.json" -Raw |
        ConvertFrom-Json
    $cellRecordPath = Join-Path $root "cell_record.paper1.v5.mechanical.json"
    if (-not (Test-Path -LiteralPath $cellRecordPath)) {
        throw "Existing cell lacks its v4 provenance record: $cellRecordPath"
    }
    ...
    if (
        ...
        $cellRecord.schema -ne "twin.factor.cell.record.paper1.v5.mechanical" -or
        $cellRecord.factor_design_sha256 -ne $script:DesignHash -or
        $cellRecord.matched_far_threshold_sha256 -ne $script:ThresholdHash
    ) {
        throw "Existing cell has conflicting v4 metadata: $RunId"
    }
...
$campaignRoot = ".\frozen\paper1_factor_campaign_v5_mechanical"
...
if ($Mode -eq "Validate") {
    ...
    "PAPER1_FACTOR_V4_FULL_PREFLIGHT_OK"
    ...
    for ($seedIndex = $SeedFrom; $seedIndex -le $SeedTo; $seedIndex++) {
        ...
        if ($Mode -eq "Run" -or $Mode -eq "All") {
            ...
            for (
                ...
                $runId = "paper1_v5mv_s$seedText`_$($level.id)"
                ...
                $cellRecord = [PSCustomObject]@{
                    schema = "twin.factor.cell.record.paper1.v5.mechanical"
                    run_id = $runId
                ...
                $cellRecord |
                    ...
                    Set-Content `
                        -LiteralPath "$cellRoot\cell_record.paper1.v5.mechanical.json" `
                        -Encoding UTF8
...
"PAPER1_FACTOR_V4_BATCH_COMPLETE"
"MODE=$Mode"
