...
def load_inputs(
    ...
    *,
    dt: float,
) -> tuple[np.ndarray, np.ndarray, int, np.ndarray, np.ndarray | None, np.ndarray]:
    ...
    return H, sigma2, n_telemetry, x_true, z_true, timestamps
...
def main(argv: list[str] | None = None) -> int:
    ...
    H, sigma2, n_telemetry, x_true, z_true, timestamps = load_inputs(
        feeder_path, truth_path, dt=args.dt
    )
    steps = resolved_steps(args.stop_time, args.dt, x_true.shape[0])
    print(
        f"power_fed preflight: steps={steps} states={H.shape[1]} "
        f"telemetry={n_telemetry} noise={args.measurement_noise} seed={args.seed}",
        flush=True,
    )
