# PAPER 1 V5.2 MECHANICAL-VALIDATION OVERLAY
#
# Purpose: estimator/schema/mechanical validation only.
# Output prefix: paper1_v5_2mv_
...
function Test-CompletedCell {
    ...
    $twin = Get-Content -LiteralPath "$root\twin\meta.json" -Raw |
        ConvertFrom-Json
    $cellRecordPath = Join-Path $root "cell_record.paper1.v5_2.mechanical.json"
    if (-not (Test-Path -LiteralPath $cellRecordPath)) {
        throw "Existing cell lacks its V5.2 provenance record: $cellRecordPath"
    }
    ...
    if (
        ...
        throw "Existing cell has conflicting V5.2 metadata: $RunId"
    }
...
$campaignRoot = ".\frozen\paper1_factor_campaign_v5_2_mechanical"
New-Item -ItemType Directory -Path $campaignRoot -Force | Out-Null

$expectedTrustMetricHash = "936DD373A2D8A2F0B905604CA4C3DE61EC2CC889BA233AA150A24F44F2926FE6"
$expectedTwinFederateHash = "9CD9FFAA32DCFE2F12ED161A8D62D2D97B2AB0B4D462FDA0E97E7F46572043A4"
...
$implementationLockPath = Join-Path `
    $campaignRoot `
    "V5_2_IMPLEMENTATION_SHA256.json"

$implementationLock = [ordered]@{
    schema = "paper1.v5_2.mechanical.implementation.v1"
    trust_metric_sha256 = $script:TrustMetricHash
    twin_fed_sha256 = $script:TwinFederateHash
    solver = "q_prior_innovation_after_measurement_bootstrap"
    normal_equations_used = $false
    jump_guard_policy = "q_process_mahalanobis"
...
if (Test-Path -LiteralPath $implementationLockPath) {
    ...
    if (
        $existingImplementationLock.schema -ne
            "paper1.v5_2.mechanical.implementation.v1" -or
        ...
        throw "V5.2 implementation lock conflicts with installed files."
    }
}
else {
    $implementationLock |
...
if ($Mode -eq "Validate") {
    ...
    "PAPER1_V5_2_MECHANICAL_PREFLIGHT_OK"
    ...
    for ($seedIndex = $SeedFrom; $seedIndex -le $SeedTo; $seedIndex++) {
        ...
        if ($Mode -eq "Run" -or $Mode -eq "All") {
            ...
            for (
                ...
                $runId = "paper1_v5_2mv_s$seedText`_$($level.id)"
                ...
                $cellRecord = [PSCustomObject]@{
                    schema = "twin.factor.cell.record.paper1.v5_2.mechanical"
                    run_id = $runId
                    ...
                    estimator_version = "paper1_v5_2"
                    trust_metric_sha256 = $script:TrustMetricHash
                    twin_fed_sha256 = $script:TwinFederateHash
                    estimator_solver = "q_prior_innovation_after_measurement_bootstrap"
                    normal_equations_used = $false
                    jump_guard_policy = "q_process_mahalanobis"
                ...
                $cellRecord |
                    ...
                    Set-Content `
                        -LiteralPath "$cellRoot\cell_record.paper1.v5_2.mechanical.json" `
                        -Encoding UTF8
