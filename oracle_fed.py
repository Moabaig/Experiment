...
MAX_EVENT_COLUMNS = {
    "s", "s_lmax", "s_trace", "r", "chi2", "huber", "lnr",
    "u_lmax", "u_trace", "u_lmax_mean", "u_trace_mean",
    "sB1", "sB2", "s_gated_lmax", "s_gated_trace", "b2",
    "loss_quantile", "x_error_norm",
}


def _constant_event_value(block: pd.DataFrame, column: str) -> Any:
    values = block[column]
    if values.nunique(dropna=False) != 1:
        raise ValueError(f"{column} changes within event {block['event_id'].iloc[0]}")
    return values.iloc[0]


def aggregate_events(frame: pd.DataFrame) -> pd.DataFrame:
    """Apply field-specific event semantics.

    High-is-bad detector statistics use a maximum. Trust ``T`` and coverage
    use a minimum. Boolean state/solver fields are summarized explicitly.
    A blanket numeric maximum is invalid because it turns event trust into
    ``max(T)`` even though ``T = exp(-s)`` and the event score is ``max(s)``.
    """
    identity = {
    ...
    for event_id, block in frame.groupby("event_id", sort=True):
        row: dict[str, Any] = {
            key: _constant_event_value(block, key)
            for key in identity
        ...
        if "T" in block.columns:
            row["T"] = float(pd.to_numeric(block["T"], errors="raise").min())
        for column in ("b1", "n_rx", "n_rx_telemetry"):
            if column in block.columns:
                value = pd.to_numeric(block[column], errors="raise").min()
                row[column] = int(value) if column.startswith("n_rx") else float(value)
        if "held" in block.columns:
            held = block["held"].astype(bool)
            row["held"] = float(held.mean())
            row["held_any"] = bool(held.any())
        if "solve_exact" in block.columns:
            solved = block["solve_exact"].astype(bool)
            row["solve_exact"] = bool(solved.all())
            row["solve_exact_fraction"] = float(solved.mean())
        if "floor_kind" in block.columns:
            kinds = block["floor_kind"].dropna().astype(str).unique()
            row["floor_kind"] = (
                kinds[0] if len(kinds) == 1 else "mixed" if len(kinds) else None
            )

        if "s" in row and "T" in row:
            expected_T = math.exp(-min(float(row["s"]), 745.0))
            if not math.isclose(row["T"], expected_T, rel_tol=1.0e-12, abs_tol=1.0e-300):
                raise ValueError(
                    f"event {event_id} violates T=exp(-max(s)); "
                    "step score/trust aggregation is inconsistent"
                )
        rows.append(row)
...
def main(argv: list[str] | None = None) -> int:
    ...
    meta: dict[str, Any] = {
        ...
        "event_aggregation": {
            "schema": "oracle.event-aggregation.v2",
            "high_is_bad": "max",
            "T": "min",
            "b1_n_rx": "min",
            "held": "mean_with_any_flag",
            "solve_exact": "all_with_fraction",
        },
