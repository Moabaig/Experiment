...
$bandwidthLevel = if ($cfg.ContainsKey('BANDWIDTH_LEVEL')) {
    $cfg['BANDWIDTH_LEVEL']
} else {
    'bw04_oracle'
}

$bandwidthCapText = if ($cfg.ContainsKey('BANDWIDTH_CAP_BPS')) {
    $cfg['BANDWIDTH_CAP_BPS']
} else {
    '1000000000000'
}
...
$bandwidthCap = 0.0
if (
    $bandwidthLevel -notmatch '^[A-Za-z0-9_.-]+$' -or
    -not [double]::TryParse($bandwidthCapText, [ref]$bandwidthCap) -or
    [double]::IsNaN($bandwidthCap) -or
    [double]::IsInfinity($bandwidthCap) -or
    $bandwidthCap -le 0.0
) {
    throw 'BANDWIDTH_LEVEL or BANDWIDTH_CAP_BPS is invalid.'
}
...
if (Test-Path -LiteralPath $runRoot) {
    throw "Run directory already exists; refusing to overwrite: $runRoot"
}
if (Test-Path -LiteralPath $logPath) {
    throw "Run log already exists; refusing to overwrite: $logPath"
}
...
Write-Host (
    "Starting four-federate run: $runId " +
    "bandwidth_level=$bandwidthLevel " +
    "bandwidth_cap_bps=$bandwidthCapText"
)
