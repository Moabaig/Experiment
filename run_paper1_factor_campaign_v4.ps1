...
function Test-CompletedCell {
    ...
    $twin = Get-Content -LiteralPath "$root\twin\meta.json" -Raw |
        ConvertFrom-Json
    $cellRecordPath = Join-Path $root "cell_record.paper1.v4.json"
    ...
    if (
        ...
        $cellRecord.schema -ne "twin.factor.cell.record.paper1.v4" -or
...
$campaignRoot = ".\frozen\paper1_factor_campaign_v4"
    ...
    for ($seedIndex = $SeedFrom; $seedIndex -le $SeedTo; $seedIndex++) {
        ...
        if ($Mode -eq "Run" -or $Mode -eq "All") {
            ...
            for (
                ...
                $runId = "paper1_s$seedText`_$($level.id)"
                ...
                $cellRecord |
                    ...
                    Set-Content `
                        -LiteralPath "$cellRoot\cell_record.paper1.v4.json" `
                        -Encoding UTF8
